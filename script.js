...
/* Coded intro — fallback. Halo-ring reveal -> tagline scene -> fade out. */
...
/* Real video intro — drives the native <video id="intro-video"> embedded in the HTML. */
function playVideoIntro(video) {
    video.addEventListener('ended', finishIntro);
    video.addEventListener('error', () => {
        // Bad or unreachable video file — fall back to the coded intro rather than a black screen
        playCodedIntro();
    });

    // Safety net in case autoplay is blocked or the video never fires "ended".
    // Sized off the video's real duration (+2s buffer) once known, so it never
    // fires before the video would end naturally. Falls back to 15s if
    // metadata never loads (e.g. the request stalls).
    let safetyTimerId = setTimeout(finishIntro, 15000);
    introTimers.push(safetyTimerId);
    video.addEventListener('loadedmetadata', () => {
        if (isFinite(video.duration) && video.duration > 0) {
            clearTimeout(safetyTimerId);
            introTimers = introTimers.filter((t) => t !== safetyTimerId);
            safetyTimerId = setTimeout(finishIntro, (video.duration + 2) * 1000);
            introTimers.push(safetyTimerId);
        }
    });

    // Autoplay can be blocked (e.g. browsers without user interaction yet);
    // catch the rejected play() promise and fall back gracefully instead of
    // leaving a frozen black frame behind.
